	...
	*/
	...
	public:
		...
		*/
		...
		TimeSeriesBase(const Time& tBegin, ValueIterator begin, ValueIterator end) {
			Time t = tBegin;
			while (begin != end)
				values_[t++] = *(begin++);
		}
		...
		// Reverse iterators
		// The following class makes compilation fail for the code that calls rbegin or rend with a 
		// container that does not support reverse iterators. 
		// All the rest TimeSeries class features should compile and work for this type of containers. 
		template<class container, class iterator_category>
		struct reverse {
			typedef boost::reverse_iterator<typename container::const_iterator> const_reverse_iterator;
		...
		// This class defines reverse iterator features via container's native calls.
		...
		// The following typedef enables reverse iterators for bidirectional_iterator_tag category.
		typedef typename boost::mpl::if_ <
			boost::mpl::or_ <
				boost::is_same<iterator_category, std::bidirectional_iterator_tag>,
				boost::is_base_of<std::bidirectional_iterator_tag, iterator_category> >,
		...
		typedef boost::function1<Time, const container_value_type &> projection_time;
		typedef boost::function1<T, const container_value_type &> projection_value;
		...
		const_value_iterator cbegin_values() const { return const_value_iterator(cbegin(), get_value); }
		const_value_iterator cend_values() const { return const_value_iterator(cend(), get_value); }
		const_reverse_value_iterator crbegin_values() const { return const_reverse_value_iterator(crbegin(), get_value); }
		const_reverse_value_iterator crend_values() const { return const_reverse_value_iterator(crend(), get_value); }
		...
		const_reverse_time_iterator crbegin_time() const { return const_reverse_time_iterator(crbegin(), get_time); }
		const_reverse_time_iterator crend_time() const { return const_reverse_time_iterator(crend(), get_time); }
		//@}
		...
		static const Time & get_time (const container_value_type &v) { return v.first; }
	...
	public:
		...
		typedef typename Base::const_value_iterator const_value_iterator;
		typedef typename Base::const_time_iterator const_time_iterator;
	...
	inline typename TimeSeriesBase<T,Time,C>::const_iterator
		TimeSeriesBase<T,Time,C>::cbegin() const {
			return values_.begin();
	}

	template <class T, class Time, class C>
	inline typename TimeSeriesBase<T,Time,C>::const_iterator
		TimeSeriesBase<T,Time,C>::cend() const {
			return values_.end();
	}
	...
	std::vector<Time> TimeSeriesBase<T,Time,C>::times() const {
		std::vector<Time> v;
		v.reserve(size());
		std::transform(cbegin(), cend(), std::back_inserter(v), TimeSeriesBase<T,Time,C>::get_time);
		return v;
	}
	...
	std::vector<T> TimeSeriesBase<T,Time,C>::values() const {
		std::vector<T> v;
		v.reserve(size());
		std::transform(cbegin(), cend(), std::back_inserter(v), TimeSeriesBase<T,Time,C>::get_value);
		return v;
	}
}
